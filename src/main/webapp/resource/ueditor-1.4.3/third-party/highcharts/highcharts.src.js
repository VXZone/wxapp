...
Axis.prototype = {
	...
	init: function (chart, userOptions) {
		...
		// Initial categories
		axis.categories = options.categories || type === 'channel';
	...
	setTickPositions: function (secondPass) {
		...
		// into acommentCount, we do this after computing tick interval (#1337).
	...
	render: function () {
		...
		if (stackLabelOptions && stackLabelOptions.enabled) {
			var stackKey, oneStack, stackChannel,
			...
			for (stackKey in stacks) {
				...
				for (stackChannel in oneStack) {
					oneStack[stackChannel].render(stackTotalGroup);
				}
			}
		}
		// End stacked totals
...
Series.prototype = {
	...
	translate: function () {
		...
		for (i = 0; i < dataLength; i++) {
			...
			// some API data
			point.channel = categories && categories[point.x] !== UNDEFINED ?
...
var ColumnSeries = extendClass(Series, {
	...
	getColumnMetrics: function () {
		...
		var channelWidth = mathMin(
				mathAbs(xAxis.transA) * (xAxis.ordinalSlope || options.pointRange || xAxis.closestPointRange || 1), 
				xAxis.len // #1535
			),
			groupPadding = channelWidth * options.groupPadding,
			groupWidth = channelWidth - 2 * groupPadding,
			...
			pointXOffset = pointPadding + (groupPadding + colIndex *
				pointOffsetWidth - (channelWidth / 2)) *
				(reversedXAxis ? -1 : 1);
