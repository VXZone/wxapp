...
<div id="bread_crumb">
  <ul class="clearfix">
   ...
   <li><a href="${g.domain}/channels/${post.channel.name}">${post.channel.name}</a></li>
...
<div class="post_wrap clearfix">
  <div class="post">
   ...
   <div class="post_content">
     <c:choose>
       <c:when test="${!sf:isUser()&&post.status=='secret'}">由于作者设置了权限，你没法阅读此文，请与作者联系</c:when>
       <c:otherwise>
         ${post.content}
  ...
  <div class="meta">
    <ul>
     ...
     <c:if test="${post.type=='post'}">
       <li class="post_channel"><a href="${g.domain}/channels/${post.channel.name}">${post.channel.name}</a></li>
     </c:if>
     <li class="post_author">
       <a title="由${post.user.username}发布" href="#">${post.user.username}</a>
     </li>
