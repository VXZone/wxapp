         ...
         <div class="col-sm-3 .col-md-3">
           <div class="panel panel-default">
             ...
             <div class="panel-body">
               <div class="form-group">
                 ...
                 <select class="form-control" id="channel">
                   <c:forEach items="${channels}" var="channel" begin="1">
                     <option value="${channel.id}" ${post.channel.id==channel.id?'selected':''}>
                        ├─<c:if test="${channel.level==3}">└─</c:if>${channel.name}
                      </option>
                   </c:forEach>
                 </select>
               </div>
               <div class="form-group">
                 <label for="status">公开度</label><br/>
                 <label class="radio-inline">
                    <input type="radio" name="status" value="publish" ${post.status=='publish'?'checked':''}>公开
                 </label>
                 <label class="radio-inline">
                    <input type="radio" name="status" value="secret" ${post.status=='secret'?'checked':''}>隐藏
                 </label>
               </div>
               <div class="form-group">
               ...
               <div class="form-group">
                 <label for="commentStatus">是否允许评论</label><br>
                 <label class="radio-inline">
                    <input type="radio" name="commentStatus" value="open" ${post.commentStatus=='open'?'checked':''}>是
                 </label>
                 <label class="radio-inline">
                    <input type="radio" name="commentStatus" value="close" ${post.commentStatus=='close'?'checked':''}>否
                 </label>
               </div>
             </div>
