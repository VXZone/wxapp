               ...
               <c:forEach items="${page.content}" var="post">
                 <tr id="edit-${post.id}">
                    <td><strong><a class="post-title" target="_blank" href="../../posts/${post.id}">${post.title}</a>
                        <c:if test="${post.status=='secret'}"> - 私密</c:if>
                     </strong>
                     <span class="hide"><i class="post-ps">${post.status}</i>
                          <i class="post-cs">${post.commentStatus}</i><i class="post-tags">${z:join(post.tags,',')}</i></span>
                     ...
                     </div></td><td class="post-author">${post.user.username}</td>
                     <td class="post-channel">${post.channel.name}</td>
                     <td class="post-ctime"><fmt:formatDate value="${post.createTime}" pattern="yyyy-MM-dd" /></td>
                     <td>${post.viewCount} views</td>
               ...
               <tr id="edit-row">
                 <td colspan="6">
                  <div class="edit-col-left">
                    ...
                    <span><label>公开度</label><input type="radio" name="status" value="publish" >公开&nbsp;
                          <input type="radio" name="status" value="secret" >隐藏</span>
                  </div>
                  <div class="edit-col-center">
                    <h5>分类目录</h5>
                    <ul>
                     <c:forEach items="${channels}" var="channel" begin="1">
                       <li><input type="radio" name="channel" value="${channel.id}" />${channel.name}</li>
                     </c:forEach>
                    </ul>
                  </div>
                  <div class="edit-col-right">
                    ...
                    <span><label>允许评论</label><input type="radio" name="commentStatus" value="open">是&nbsp;
                           <input type="radio" name="commentStatus" value="close">否</span>
                  </div>
                  <p>
