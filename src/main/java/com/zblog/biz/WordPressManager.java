...
public class WordPressManager{
  ...
  private void importPost(MapContainer post, User user, String wpdoamin, Map<String, Upload> links){
    ...
    Channel channel = channelService.loadByName(post.getAsString("channel"));
    if(channel != null){
      p.setChannelId(channel.getId());
    }else{
      // TODO 设置默认栏目
      p.setChannelId(0);
      //p.setChannelId(optionsService.getOptionValue(OptionConstants.DEFAULT_CHANNEL_ID));
    }
    ...
    p.setDescription(cleanTxt.length() > PostConstants.EXCERPT_LENGTH ? cleanTxt.substring(0, PostConstants.EXCERPT_LENGTH)
            : cleanTxt);
