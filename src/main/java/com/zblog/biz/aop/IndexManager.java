...
public class IndexManager{
  ...
  public void update(Post post, boolean affect){
    if(PostConstants.TYPE_POST.equals(post.getType()) && affect){
      SearchEnginer.postEnginer().update(new Term("id", post.getId().toString()), convert(post));
    }
  }

  public void remove(Integer postId, String postType){
    if(PostConstants.TYPE_POST.equals(postType))
      SearchEnginer.postEnginer().delete(new Term("id", postId.toString()));
  }
  ...
  public PageModel<MapContainer> search(String word, int pageIndex){
    ...
    builder.addShould("title", word).addShould("description", word);
    builder.addLighter("title", "description");
  ...
  private Document convert(Post post){
    ...
    /* 用jsoup剔除html标签 */
    doc.add(new Field("description", JsoupUtils.plainText(post.getContent()), LuceneUtils.searchType()));
