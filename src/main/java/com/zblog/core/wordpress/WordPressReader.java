    ...
    try{
      ...
      for(int i = 0; i < items.getLength(); i++){
        ...
        if("attachment".equals(itemType)){
          ...
          NodeList channels = item.getElementsByTagName("channel");
          for(int j = 0; j < channels.getLength(); j++){
            Element channel = (Element) channels.item(j);
            String channelDomain = channel.getAttribute("domain");
            if("post_tag".equals(channelDomain)){
              map.putIfAbsent("tags", new ArrayList<String>()).add(channel.getTextContent());
            }else if("channel".equals(channelDomain)){
              map.put("channel", channel.getTextContent());
            }
          }
